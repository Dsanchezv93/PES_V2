...
import { ParisRule } from "./paris";
import { GroupIndexes, GroupingConfiguration } from "../grouping-configuration";

void describe("ParisRule", () => {
  const rule = new ParisRule();
  let itemMock: Item;
  ...
  void it("should apply the rule to the input item", () => {
    ...
    assert.strictEqual(itemMock.classificationGroup, "1P-Paris-REGULAR-facility2-SAME_DAY-DELIVERY-2021-01-01-2021-01-01-AM1");
  });
  ...
  void it("should apply the rule to the input item using priority 0", () => {
    ...
    rule.apply(itemMock, config);
    assert.strictEqual(itemMock.classificationGroup, `1P-Paris-REGULAR-facility1-SCHEDULED-DELIVERY-2021-01-01-2021-01-01-TH`);
  });
  ...
  void it("should apply the rule to the input item using priority 0 with optional priority properties", () => {
    ...
    rule.apply(itemMock, config);
    assert.strictEqual(itemMock.classificationGroup, `1P-Paris-REGULAR-facility1-SCHEDULED-COLLECT-IN_STORE-2021-01-01-2021-01-01-TH`);
  });
  ...
  void it("should not match a non-Paris item", () => {
    itemMock.sellerType = SellerType.Marketplace;
    const result = rule.matches(itemMock);
    assert.strictEqual(result, false);
  });
  ...
  void it("should tag the item correctly", () => {
    const indexes: GroupIndexes = {
      ...
      shippingMethodIndex: ShippingMethod.Scheduled,
    ...
    assert.strictEqual(itemMock.classificationGroup, "1P-SellerName-REGULAR-facility1-SCHEDULED-DELIVERY-2021-01-01-2021-01-01-TH");
  });
